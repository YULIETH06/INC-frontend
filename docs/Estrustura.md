...
| `FilePreviewCard.tsx` | Componente reutilizable encargado de mostrar visualmente un archivo adjunto previamente cargado. Presenta la información básica del archivo y proporciona la acción correspondiente para visualizarlo, manteniendo separada la presentación del documento de la lógica específica del módulo que lo utiliza. | Puede reutilizarse en diferentes módulos que requieran visualizar archivos adjuntos o evidencias documentales. |
...
| Función | Descripción |
| ------- | ----------- |
| `getPersonnelCandidateValidations()` | Consulta los candidatos preseleccionados disponibles para el módulo de validación junto con el estado resumido del proceso. Talento Humano consulta el conjunto general y los creadores autorizados consultan únicamente candidatos de sus propias requisiciones. |
| `getPersonnelCandidateValidationDetail(candidateId)` | Consulta el detalle de un candidato, la requisición, la revisión exacta del perfil, la validación existente y la Evaluación Técnica cuando aplica. |
...
| `getPersonnelCandidateSubmissionBatches(requisitionId)` | Consulta las fotografías históricas generadas en cada cierre del cargue. Cada registro contiene el número de cargue, fecha, usuario que cerró y la lista inmutable de candidatos con su orden, identificación y nombre. |
| `preselectPersonnelRequisitionCandidates(requisitionId, data)` | Confirma la preselección de uno o varios candidatos enviando `candidateIds`. La acción solo puede realizarla el creador de la requisición cuando esta está aprobada y el cargue está cerrado; una preselección confirmada queda registrada de forma permanente. |
